...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        super().__init__()
        self.width = width
        self.layers = layers #12 layers，768 or 512 channels
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text): #(N,77)
        x = self.token_embedding(text).type(self.dtype_)  ##（batch×77×512）  # [batch_size, n_ctx, d_model]

        x = x + self.positional_embedding.type(self.dtype_) #（77*512)
